import type { MusicCommand, MusicReply } from "./music";
...
export interface ClientToServerEvents {
  "music:request": (
    data: { code: string; command?: MusicCommand; revision?: number },
    reply: (data: MusicReply) => void,
  ) => void;
  ...
  "room:leave": (
    data: { roomId: string },
    reply: (data: { ok: true } | { ok: false; error: string }) => void,
  ) => void;
  ...
  // Chat events
  "chat:send": (
    data: { roomId: string; body: string; clientIdempotencyKey: string },
    reply: (
      data: { ok: true; messageId: number } | { ok: false; error: string },
    ) => void,
  ) => void;
  "chat:history": (
    data: { roomId: string; beforeId?: number; limit?: number },
    reply: (
      data:
        | {
            ok: true;
  ...
  "chat:mark_read": (
    data: { roomId: string; upToMessageId: number },
    reply: (data: { ok: true } | { ok: false; error: string }) => void,
  ) => void;

  // Media queue events
  "queue:add": (
    data: { roomId: string; mediaId: number },
    reply: (
      data: { ok: true; itemId: number } | { ok: false; error: string },
    ) => void,
  ) => void;
  "queue:remove": (
    data: { roomId: string; itemId: number },
    reply: (data: { ok: true } | { ok: false; error: string }) => void,
  ) => void;
  "queue:reorder": (
    data: { roomId: string; itemId: number; newPosition: number },
    reply: (data: { ok: true } | { ok: false; error: string }) => void,
  ) => void;
  "queue:clear": (
    data: { roomId: string },
  ...
  "playback:play": (
    ...
    reply: (
      data: { ok: true; revision: number } | { ok: false; error: string },
    ) => void,
  ) => void;
  "playback:pause": (
    data: { roomId: string; positionSeconds: number; revision: number },
    reply: (
      data: { ok: true; revision: number } | { ok: false; error: string },
    ) => void,
  ) => void;
  ...
  "playback:change_media": (
    data: { roomId: string; mediaId: number | null; revision: number },
    reply: (
      data: { ok: true; revision: number } | { ok: false; error: string },
    ) => void,
  ) => void;

...
import { validateBody, roomValidationRules } from '../middleware/validation';
import { asyncHandler, AppError } from '../utils/errors';

const router = Router();

// Create Room
router.post('/api/rooms/create', requireAuth, validateBody(roomValidationRules.create), asyncHandler(async (req, res) => {
  const roomCode = generateRoomCode();
  const { name, type, password, privacy } = req.body as { 
    name: string; 
    type: 'video' | 'music'; 
    password?: string; 
  ...
  const result = await pool.query(
    ...
    [roomCode, name, type, req.user!.id, passwordHash, privacy]
  );
...
// List user's rooms
router.get('/api/rooms', requireAuth, asyncHandler(async (req, res) => {
  const result = await pool.query(
...
// Join Room
router.post('/api/rooms/join/:code', requireAuth, validateBody(roomValidationRules.join), asyncHandler(async (req, res) => {
  ...
  if (room.status !== 'active') {
    throw AppError.badRequest('Room is not active');
  }
  ...
  if (room.privacy === 'private' && room.password_hash) {
    if (!password) {
      throw AppError.unauthorized('Password required', { requiresPassword: true });
    }
...
// Get room details
router.get('/api/rooms/:roomCode', requireAuth, asyncHandler(async (req, res) => {
...
// Leave Room
router.post('/api/rooms/:roomCode/leave', requireAuth, asyncHandler(async (req, res) => {
  const code = req.params.roomCode;
...
router.delete('/api/rooms/:roomCode', requireAuth, asyncHandler(async (req, res) => {
  const code = req.params.roomCode;
  ...
  if (room.host_id !== req.user!.id) {
    throw AppError.forbidden('Only the host can close the room');
  }
...
router.delete('/api/rooms/:roomCode/members/:userId', requireAuth, asyncHandler(async (req, res) => {
  const code = req.params.roomCode;
  ...
  const room = roomResult.rows[0];
  if (room.host_id !== req.user!.id) {
    throw AppError.forbidden('Only the host can remove members');
  }

  if (targetUserId === room.host_id) {
    throw AppError.badRequest('Cannot remove the host');
  }

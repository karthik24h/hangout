...
router.post(
  '/api/rooms/create',
  requireAuth,
  validateBody(roomValidationRules.create),
  asyncHandler(async (req, res) => {
    ...
    // Add host as member
    await pool.query(`INSERT INTO room_members (room_id, user_id, role) VALUES ($1, $2, 'host')`, [
      room.id,
      req.user!.id,
    ]);
...
router.post(
  '/api/rooms/join/:code',
  requireAuth,
  validateBody(roomValidationRules.join),
  asyncHandler(async (req, res) => {
    const code = req.params.code;
    const { password } = req.body as { password?: string };

    const roomResult = await pool.query(`SELECT * FROM rooms WHERE room_code = $1`, [code]);
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const roomResult = await pool.query(`SELECT id, host_id FROM rooms WHERE room_code = $1`, [
      code,
    ]);
    if (roomResult.rows.length === 0) {
      throw AppError.notFound('Room not found');
    ...
    await pool.query(`UPDATE rooms SET status = 'closed', closed_at = NOW() WHERE id = $1`, [
      room.id,
    ]);
...
router.delete(
  '/api/rooms/:roomCode/members/:userId',
  requireAuth,
  asyncHandler(async (req, res) => {
    const code = req.params.roomCode;

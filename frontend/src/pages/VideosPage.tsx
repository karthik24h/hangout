    ...
    <div className="dashboard videos-page">
      ...
      <div className="dash-shell">
        ...
        <main className="dash-main videos-main">
          ...
          {roomCode ? <section className="videos-stage" aria-labelledby="stage-title"><div className="videos-stage__header"><div><p className="videos-eyebrow">SHARED SCREEN</p><h2 id="stage-title">Ready when you are.</h2></div><span className="videos-status"><span className="videos-live-dot" /> Waiting for media</span></div><div className="videos-player videos-player--empty"><div className="videos-player__icon"><Icon name="video" /></div><h3>{media ? media.title : 'No video selected'}</h3>{media && <video controls preload="metadata" src={media.url} aria-label={media.title} onLoadedMetadata={event => { event.currentTarget.volume = 1; event.currentTarget.muted = false; }} />}<p>Add a supported video link from the room to start watching together.</p><button className="videos-button videos-button--secondary" onClick={() => setAddMedia(true)}><Icon name="plus" /> Add media</button></div><div className="videos-stage__footer"><span>Playback sync will appear here once media is added.</span><span>Room code <strong>{roomCode}</strong></span></div></section> : <section className="videos-empty-hero"><div className="videos-empty-hero__art"><div className="videos-art-orbit videos-art-orbit--one" /><div className="videos-art-orbit videos-art-orbit--two" /><Icon name="video" /></div><div><p className="videos-eyebrow">YOUR WATCHSPACE</p><h2>Nothing queued yet.</h2><p>Create a room for a private watch party, or join one with an invite code. Your rooms and playback state will appear here.</p><div className="videos-actions"><button className="videos-button videos-button--primary" onClick={() => setCreate(true)}><Icon name="plus" /> Create a room</button><button className="videos-button videos-button--secondary" onClick={() => setJoin(true)}>Join with code</button></div></div></section>}
